...
export class DisputesController {
  ...
  @Roles(UserRole.CARD_MEMBER, UserRole.MERCHANT, UserRole.ANALYST)
  @ApiOperation({
    summary: 'Get audit log entries for a dispute',
  })
  @ApiOkResponse({ description: 'Case audit log entries' })
  @ApiUnauthorizedResponse({ description: 'Missing or invalid bearer token' })
  @ApiForbiddenResponse({
    description: 'User cannot read this dispute audit log',
  })
  @ApiNotFoundResponse({ description: 'Dispute case not found' })
  auditLog(
    @Param('caseId', ParseUUIDPipe) caseId: string,
    ...
    return this.auditService.findForCase(caseId, user);
  }
